...
import {
  Box, Container, Typography, Grid, Card, CardContent, TextField, Button, Chip,
  InputAdornment, Avatar, Paper, MenuItem, Select, FormControl, InputLabel,
  IconButton, useTheme, useMediaQuery, Stack, Skeleton, Tabs, Tab,
  Pagination, Drawer, AppBar, Toolbar, CssBaseline
} from '@mui/material';
import SearchIcon from '@mui/icons-material/Search';
import LocationOnIcon from '@mui/icons-material/LocationOn';
...
import CloseIcon from '@mui/icons-material/Close';
import MenuIcon from '@mui/icons-material/Menu';
import BusinessCenterIcon from '@mui/icons-material/BusinessCenter'; // Fixed: was missing
import { categoryDefinitions } from './Categories';
...
function JobList({ user }) {
  ...
  return (
    <>
      ...
      <Container sx={{ pt: isMobile ? 8 : 4, pb: 10 }}>
        {/* Search */}
        <Paper sx={{ p: 2, mb: 3, borderRadius: 3 }}>
          <TextField
            fullWidth
        ...
        >
          <Tab label={`All (${jobs.length})`} />
          <Tab label={`Applied (${applications.length})`} disabled={!user} />
          <Tab label={`Saved (${savedJobs.length})`} disabled={!user} />
        </Tabs>
        ...
        {loading ? (
          ...
          <Box textAlign="center" py={5}>
            <BusinessCenterIcon sx={{ fontSize: 80, color: 'grey.400', mb: 2 }} />
            <Typography color="text.secondary" variant="h6">
              {activeTab === 0 ? 'No jobs found. Try adjusting filters!' : 'Nothing here yet.'}
            </Typography>
            {!user && activeTab === 0 && (
              <Button variant="contained" sx={{ mt: 2 }} onClick={() => navigate('/signin')}>
                Sign In to Apply
              </Button>
            )}
          </Box>
                ...
                <Grid item xs={12} key={job.id}>
                  <Card sx={{ position: 'relative' }}>
                    <IconButton
                      ...
                      onClick={() => user ? toggleSaveJob(job.id) : navigate('/signin')}
                    >
                      {savedJobs.includes(job.id) ? <BookmarkIcon color="primary" /> : <BookmarkBorderIcon />}
                    </IconButton>
                    <CardContent>
                      ...
                      {job.userId === user?.uid ? (
                        ...
                        <Button
                          fullWidth
                          variant="contained"
                          onClick={() => user ? navigate(`/apply/${job.id}`) : navigate('/signin')}
                        >
